...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NodeServiceNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NodeServiceNetworkObject.Length > 0 && NodeServiceNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NodeServiceNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NodeServiceBehavior>();
						}
					}
			...
			else if (obj is RandomMovingNPCNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (RandomMovingNPCNetworkObject.Length > 0 && RandomMovingNPCNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(RandomMovingNPCNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<RandomMovingNPCBehavior>();
						}
					}
			...
			else if (obj is TokenPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TokenPlayerNetworkObject.Length > 0 && TokenPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TokenPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<TokenPlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNodeService instead, its shorter and easier to type out ;)")]
		public NodeServiceBehavior InstantiateNodeServiceNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NodeServiceNetworkObject[index]);
			var netBehavior = go.GetComponent<NodeServiceBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NodeServiceBehavior>().networkObject = (NodeServiceNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateRandomMovingNPC instead, its shorter and easier to type out ;)")]
		public RandomMovingNPCBehavior InstantiateRandomMovingNPCNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RandomMovingNPCNetworkObject[index]);
			var netBehavior = go.GetComponent<RandomMovingNPCBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<RandomMovingNPCBehavior>().networkObject = (RandomMovingNPCNetworkObject)obj;
		...
		[Obsolete("Use InstantiateTokenPlayer instead, its shorter and easier to type out ;)")]
		public TokenPlayerBehavior InstantiateTokenPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TokenPlayerNetworkObject[index]);
		...
		/// <summary>
		/// Instantiate an instance of NodeService
		/// </summary>
		/// <returns>
		/// A local instance of NodeServiceBehavior
		/// </returns>
		/// <param name="index">The index of the NodeService prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NodeServiceBehavior InstantiateNodeService(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NodeServiceNetworkObject[index]);
			var netBehavior = go.GetComponent<NodeServiceBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of RandomMovingNPC
		/// </summary>
		/// <returns>
		/// A local instance of RandomMovingNPCBehavior
		/// </returns>
		/// <param name="index">The index of the RandomMovingNPC prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public RandomMovingNPCBehavior InstantiateRandomMovingNPC(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RandomMovingNPCNetworkObject[index]);
			var netBehavior = go.GetComponent<RandomMovingNPCBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of TokenPlayer
		/// </summary>
		/// <returns>
		/// A local instance of TokenPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the TokenPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public TokenPlayerBehavior InstantiateTokenPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TokenPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<TokenPlayerBehavior>();
